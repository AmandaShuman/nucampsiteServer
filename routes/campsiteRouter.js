...
.get((req, res, next) => { //don't need to verify get request because it is a read-only operation
  ...
  .delete(authenticate.verifyUser, authenticate.verifyAdmin, (req, res, next) => {  //dangerous operation so don't want to just let anyone do it
  Campsite.deleteMany()
...
campsiteRouter.route('/:campsiteId/comments/:commentId') //will handle requests of a specific comment id of a specific campsite id
  ...
  .put(authenticate.verifyUser, (req, res, next) => { //we want to update existing data - don't want to update author, cannot update id or timestamp fields, so only want to update rating and comment text fields
    Campsite.findById(req.params.campsiteId)
      .then(campsite => {
